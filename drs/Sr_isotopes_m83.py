#/ Type: DRS
#/ Name: Sr Isotopes No Mass 82
#/ Authors: Joe Petrus and Bence Paul
#/ Description: A Sr isotopes DRS with no reference to mass 82. Instead it uses mass 83 for CaAr interference corrections
#/ References: None
#/ Version: 1.0
...
def runDRS():
	...
	PSrCaAr86 = SrCaAr86 - (CaAr83 * 0.00004 / 0.00135) / np.power((85.9160721 / 82.92115), PFract)
	PSrCaAr88 = SrCaAr88 - (CaAr83 * 0.00187 / 0.00135) / np.power((87.9149151 / 82.92115), PFract)
	...
	Ca84 = CaAr83 * 15.45185 / np.power((83.917989 / 82.92115), CaArFract)
	Sr84 = SrCaAr84 - Ca84
	Sr86 = SrCaAr86 - ((CaAr83 * 0.00004 / 0.00135) / np.power((85.9160721 / 82.92115), Fract))
	Sr88 = SrCaAr88 - ((CaAr83 * 0.00187 / 0.00135) / np.power((87.9149151 / 82.92115), Fract))
	...
	inRun8283_Ratio = CaAr83 / CaAr83	* mask	# NOTE: this is not used with no mass 82 measured. Normally this is a measure of REE vs. CaAr contributions. If pure CaAr = 4.9. If pure REE it's not entirely predictable, based on solution analysis of Indian perovsite = 3.6, Indian WR = 2.4.
